...
_OG_PAGES = {
    "":            ("Austin 311 — Real data on what your city is doing",
                    "Live maps and trends across 311 reports, APD crime, crashes, parking, courts, and more — refreshed daily."),
    ...
    "parking":     ("Austin 311 — Parking Enforcement Map",
                    "Citywide 311 parking enforcement reports — last 30/60/90 days, refreshed weekly."),
...
# Fallback for slugs that have no entry above (most map pages). Those pages are
# regenerated by weekly.yml, hence "weekly" — the landing page entry ("") is
# deliberately separate because the homepage counts still refresh daily.
_OG_FALLBACK = (
    "Austin 311",
    "Live maps and trends across Austin's 311 reports, APD crime, crashes, "
    "parking, courts, and more — refreshed weekly.",
)
...
def og_meta_tags(slug: str = "") -> str:
    ...
    """
    title, desc = _OG_PAGES.get(slug, _OG_FALLBACK)
