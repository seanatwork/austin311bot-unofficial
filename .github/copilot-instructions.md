...
Unofficial Telegram bot + companion website (https://austin311.com) for exploring Austin 311 service data. Users interact via Telegram slash commands and inline buttons to query live Open311/Socrata APIs. The `docs/` folder is a GitHub Pages site with 41+ static map/trend pages regenerated by CI: maps and trends weekly (`weekly.yml`), homepage stats and day-sensitive snapshots daily (`daily.yml`).
